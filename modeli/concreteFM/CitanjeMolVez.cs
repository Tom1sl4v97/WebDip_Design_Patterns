...
namespace ttomiek_zadaca_1.ConcrreteFM
{
    public class CitanjeMolVez : OsnovneMetode
    {
                        ...
                        for (int i = 0; i < idVezova.Length; i++)
                        {
                            try
                            {
                                ...
                                if(postojeciVez != null)
                                {
                                    ...
                                    {
                                        BrojacGreske.Instance.dodajGreskeUcitavanjaCSVDatoteki("Navedeni ID veze: " + idVeza + " već postoji u popisu veza molova.");
                                    }
                                }
                                else
                                {
                                    BrojacGreske.Instance.dodajGreskeUcitavanjaCSVDatoteki("Ne postoji traženi id veza: " + idVeza + " u zapisu: " + podaciRetka[1]);
                                }

                            } catch (Exception)
                            {
                                BrojacGreske.Instance.dodajGreskeUcitavanjaCSVDatoteki("Nemoguće je parsirati id veza iz podatka: " + idVezova[i]);
                            }
                        }
                    }
                    else
                    {
                        BrojacGreske.Instance.dodajGreskeUcitavanjaCSVDatoteki("Navedeni ID mola: " + trenutniIdMola + " već postoji u popisu MolVez.");
                    }
                }
                catch (Exception e)
                {
                    BrojacGreske.Instance.dodajGreskeUcitavanjaCSVDatoteki("Neispravni redak: " + line + " u datoteci: " + NaziviDatoteka.Instance.molVezovi + " GRESKA: " + e.Message);
                }
            }
        ...
        private void filtrirajVisakVezova()
        {
            ...
            if (counter > 0)
            {
                BrojacGreske.Instance.dodajGreskeUcitavanjaCSVDatoteki("Ukupno je izbrisano " + counter + " veza iz popisa veza, jer ne posijeduju dodijeljeni kanal.");
            }
