    ...
    public class CitanjeZahtjevRezervacije : OsnovneMetode
    {
        public override void provjeriDohvacenePodatke(string[] lines)
        {
            foreach (string line in lines)
            {
                ...
                {
                    BrojacGreske.Instance.dodajNovuGesku("Neispravni redak: " + line + " u datoteci: " + NaziviDatoteka.Instance.zahtjevRezervacije + " GRESKA: " + e.Message);
                }
            }
        }
        ...
        private void provjeraVelicineBroda(int idBroda)
        {
            ...
            List<Vez> popisVezova = new List<Vez>(BrodskaLuka.Instance.dohvatiKorijen().dohvatiPopisVezova());
            List<Vez> listaVezovaPremaVrsti = popisVezova.FindAll(x => x.vrsta == odgovarajucaVrstaVeza);
        ...
        private List<Vez> dohvatiSlobodneVezove(List<Raspored> popisRasporeda, Brod podaciBroda)
        {
            ...
            List<Vez> popisVezova2 = new List<Vez>(BrodskaLuka.Instance.dohvatiKorijen().dohvatiPopisVezova());
            List<Vez> sviVezovi = new List<Vez>(popisVezova2.FindAll(x => x.vrsta == odgovarajucaVrstaVeza));
